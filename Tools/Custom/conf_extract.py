...
Modified by: elusivethreat
    - Removed need for malduck or pycryptodome by adding custom rc4 decryption function (from OALabs)
    - Added pretty_print to match output from JoeSandbox: (https://www.joesandbox.com/analysis/883911/0/html)
...
def extract_c2_buffers(stage_3_shellcode):
    ...
    for match in results:
        ...
        while True:
            ...
            # Custom rc4 func
            c2 = rc4crypt(stage_3_shellcode[c2_addr+5:c2_addr+5+len_c2], rc4_key).decode()
            c2s.append(c2)
...
def main():
    ...
    # Smoke Version
    smoke_ver = extract_version(smoke_stage_3)
    
    if len(encrypt_keys) == 1 and len(decrypt_keys) == 1:
        # Success
        print(c.GREEN + f'[+] Successfully extracted configuration from stage3 of Smoke!')
        output = {'Version': smoke_ver, 'C2 list': c2s, 'Crypto keys': {'encrypt': hex(encrypt_keys[0]), 'decrypt': hex(decrypt_keys[0])}}
        print(c.BLUE)
        pprint(output)
        print(c.RESET)
    else:
